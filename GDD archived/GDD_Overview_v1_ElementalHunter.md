...
- **Nguyên tố = Affinity của player:** Ngựa đó nhận +ATK (xem Balance & Config).
- **Nguyên tố ≠ Affinity:** Player nhận +Mana (không gắn với ngựa cụ thể).
...
- Kích thước tối đa: `maxElementQueue` (cấu hình theo player config).
...
Phần thưởng combo phụ thuộc vào Character của player (xem phần 6). Với Character hiện tại (char1):
...
- **Double Harvest:** Tile Gain Multiplier tăng lên ×2 (nhận ATK/Mana từ ô gấp đôi).
...
|---|---|
| Đáp ô cùng Affinity | Ngựa đó +30 ATK (hoặc +60 nếu Tile Gain Multiplier = ×2) |
...
Mỗi player có 1 thanh HP, giá trị khởi đầu theo Level đã chọn:
...
Mana là tài nguyên dùng để kích hoạt Ultimate:

- **Nguồn thu:** Ngựa đáp xuống ô **khác Affinity** → +10 Mana cho player (hoặc +20 nếu Tile Gain Multiplier = ×2).
- **Giới hạn:** Mana không vượt quá `manaCap`. Khi cộng Mana, hệ thống ghi nhận lượng Mana thực tế nhận được (sau khi đã bị cap) để hiển thị feedback.
- Mana **không hao hụt** theo thời gian.
...
#### 6.1 Character Hiện Tại (char1)

Hiện tại game có **1 Character** (char1 — chưa có tên chính thức), cả 2 player đều sử dụng.
...
| TILE_REWARD_ATK | +30 ATK | Nhận khi đáp ô cùng Affinity |
| TILE_REWARD_MANA | +10 Mana | Nhận khi đáp ô khác Affinity |
...
| ULTIMATE_COST_EXTRA_ROLL | 50 Mana | |
...
| Tỷ lệ sử dụng Power Roll | % lượt mà player chọn dùng Power Roll thay vì đổ ngẫu nhiên. | TBD | Số lần dùng Power Roll / tổng số lần đổ |
| Tỷ lệ sử dụng Artifact | % lần ghé Empty Tile mà player thực sự dùng Artifact (không bỏ qua). | TBD | Số lần dùng Artifact / `emptyTileVisits` |
| Tỷ lệ sử dụng Ultimate | % lượt mà Mana đủ để dùng Ultimate và player thực sự kích hoạt. | TBD | Số lần kích hoạt Ultimate / số lần đủ Mana |
...
| **Element Affinity** | Nguyên tố chủ đạo của player, được quyết định bởi Character. Đáp ô cùng Affinity → +ATK; khác Affinity → +Mana. Code: `elementAffinity` |
...
| **HP** | Máu của player. Về 0 → thua. Code: `player.hp` |
| **Mana** | Tài nguyên dùng Ultimate, tích lũy khi đáp ô khác Affinity. Code: `player.mana` |
| **manaCap** | Giới hạn Mana tối đa. Code: `player.manaCap` |
| **Ultimate** | Kỹ năng đặc biệt kích hoạt bằng Mana. Hiện tại: Extra Roll. Code: `UltimateType` |
| **Extra Roll** | Ultimate — nhận thêm 1 lượt đổ xúc xắc trong turn hiện tại. Cost: 50 Mana. Code: `ultimateExtraRolls` |
...
| **Tile Gain Multiplier** | Hệ số nhân phần thưởng nhận từ ô (ATK/Mana). Mặc định ×1, tăng lên ×2 qua Combo Tier 2. Code: `tileGainMultiplier` |
